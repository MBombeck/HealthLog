 ...
 * or an empty table is `unknown`, and `unknown` buys nothing: the caller
 * treats it exactly like `unhealthy`.
 *
 ...
 *   unhealthy — it has been tried and it is not working.
 *   unknown   — nobody has tried it yet on this instance, or the read
 *               failed. Never an offer; see the module note on failing closed.
 */
...
export async function readServerProviderHealth(
  ...
  return workedRecently ? "healthy" : "unhealthy";
}
