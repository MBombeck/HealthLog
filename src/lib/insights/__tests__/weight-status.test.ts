...
describe("generateWeightStatusForUser — v1.4.41 timeout-stub persistence", () => {
  it("persists a sentinel row keyed to today when the provider times out", async () => {
    ...
    // Provider hangs past the 20 s timeout — withTimeout cuts the race.
    vi.mocked(resolveProvider).mockResolvedValue({
      type: "anthropic",
  ...
  it("subsequent mounts short-circuit at the cache lookup and skip the race", async () => {
    ...
    const stubRow = {
      ...
      details: JSON.stringify({
        dateKey: todayKey,
        locale: "en",
        text: "Weight assessment temporarily unavailable.",
        timeout: true,
      }),
    };
    ...
    const result = await generateWeightStatusForUser("user-1", { locale: "en" });

    expect(providerCall).not.toHaveBeenCalled();
    expect(result.text).toBe("Weight assessment temporarily unavailable.");
    expect(result.cached).toBe(true);
