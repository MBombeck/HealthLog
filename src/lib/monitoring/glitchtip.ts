...
async function tryEnvelope(
  ...
  if (response.ok) return { ok: true, method: "envelope", status: response.status };
...
async function tryStoreQuery(
  ...
  if (response.ok) return { ok: true, method: "store-query", status: response.status };
  const details = await response.text().catch(() => "");
  return {
    ok: false,
    method: "store-query",
...
async function tryStoreHeader(
  ...
  if (response.ok) return { ok: true, method: "store-header", status: response.status };
  const details = await response.text().catch(() => "");
  return {
    ok: false,
    method: "store-header",
...
export async function sendGlitchtipEvent(params: {
  ...
  return {
    ...
    status: storeHeaderResult.status ?? storeQueryResult.status ?? envelopeResult.status,
    details:
