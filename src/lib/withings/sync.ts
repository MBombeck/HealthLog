...
async function getValidToken(userId: string): Promise<{
  ...
  if (connection.tokenExpiresAt.getTime() - 5 * 60 * 1000 < Date.now()) {
    try {
      ...
      console.error(`[withings] Token refresh failed for user ${userId}:`, err);
      return null;
    }
  }
...
export async function syncUserMeasurements(
  ...
  for (const m of measures) {
    ...
    try {
      ...
      console.error(`[withings] Failed to upsert measure:`, err);
    }
  }
...
export async function setupWebhook(userId: string): Promise<void> {
  ...
  try {
    ...
    console.error(
      `[withings] Webhook subscribe failed for user ${userId}:`,
      err,
    );
  }
}
