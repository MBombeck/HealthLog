...
/**
 * Workout insert payload — the shape `POST /api/measurements/batch?kind=workout`
 * will accept once that endpoint lands (v1.5). Defined here so the iOS
 * Swift session can generate its DTO against the locked contract and
 * the v1.4.26 XML import worker has the same target.
 */
