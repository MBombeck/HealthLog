 ...
 * sections so the Wave-4b panels read as one visual group.
 *
 * Layout:
...
export function TitrationSection({ medicationId }: TitrationSectionProps) {
  ...
  const dwellHint = useMemo(() => {
    ...
    if (data.escalationDue) {
      return templateFill(t("medications.titration.escalationDueHint"), {
        weeks: data.weeksOnCurrentStep,
        typical: data.currentStep.typicalWeeks,
      });
    }
    return templateFill(t("medications.titration.currentStepDwell"), {
      weeks: data.weeksOnCurrentStep,
    });
  }, [data, t]);
  ...
  const nextStepCaption = useMemo(() => {
    ...
    return templateFill(t("medications.titration.nextStepCaption"), {
  ...
  return (
    <section
      ...
      aria-labelledby="titration-heading"
      data-slot="titration-section"
    >
      <header className="flex items-center justify-between px-3 py-2.5">
        <h2
          id="titration-heading"
          className="text-foreground/85 text-sm font-medium"
        >
          {t("medications.titration.section")}
        </h2>
        {data && (
          <span
                  ...
                  return (
                    ...
                    >
                      <span className="text-muted-foreground text-[10px] uppercase tracking-wide">
                        {templateFill(t("medications.titration.stepLabel"), {
                          n: step.stepIndex + 1,
                        })}
                      </span>
