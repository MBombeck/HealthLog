...
export function PersonalRecordBadge({
  ...
  const fresh = (data ?? []).find(
    (row) =>
      row.metricSlot === null && isPRWithin30Days(row.achievedAt, dataUpdatedAt),
  );
  ...
  const pill = (
    <span
      ...
      className={cn(
        "border-dracula-green/40 bg-dracula-green/10 text-dracula-green inline-flex shrink-0 items-center rounded-full border px-1.5 py-0.5 text-[10px] leading-none font-semibold tracking-wide tabular-nums uppercase",
        className,
      )}
