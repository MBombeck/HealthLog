...
**`category` parameter** — undocumented for new additions; we leave `category` unset, which returns *real measures* (not goals). No change needed.
...
- `elevation` → `FLIGHTS_CLIMBED` (enum exists, but Withings unit is *floors*; verify — Withings docs list `elevation: Number of floors climbed`. Match!)
...
**This is good news.** Marketing copy can honestly say: *"HealthLog reads everything a Withings device records, regardless of whether you subscribe to Withings+."*
