 ...
 *   - the scoped token's source rule still holds: MANUAL, never APPLE_HEALTH;
...
describe("POST /api/measurements — named sleep stage", () => {
  ...
  it("dedupes against the batch route: same source, same id, one row", async () => {
    ...
    const all = await rows();
    expect(all).toHaveLength(1);
    expect(all[0]).toMatchObject({ sleepStage: "REM", source: "MANUAL" });
  });

  it("treats a same-reading APPLE_HEALTH row as the same segment", async () => {
    ...
    expect(((await res.json()) as Created).data.status).toBe("duplicate");
    expect(await rows()).toHaveLength(1);
  });
