...
- **Unified dashboard snapshot** — `GET /api/dashboard/snapshot` assembles the above-the-fold tile data in one round-trip from the rollup tier plus the cached briefing, so the tiles arrive together. A coverage miss returns the heavy slice as deferred rather than blocking the strip.
...
- **Measurement delta feed** — `GET /api/sync/changes` exposes a paginated, opaque-cursor delta feed with tombstones for measurements, and `/api/sync/state` is now part of the documented API and reports the sync window. The two measurement delete routes soft-delete (tombstone) rather than hard-delete, pruned past a retention window keyed to the refresh-token lifetime. A stable `errorCode` on the refresh endpoint distinguishes a revoked family from a transient failure. Migration `0096` indexes the feed's keyset order.
...
- **Dashboard widget layout accepts client-only widget ids** — saving a layout no longer rejects ids the web surface doesn't render; unknown ids round-trip rather than failing the write.
